class MegaTraderAPI {
    ...
    async triggerOrder(logDetails) {
        ...
        const { side, price, tkn, executionQty, triggerPrice, index } = logDetails;
        if (!tkn) {
        ...
        const isBSE = index === 'SENSEX' || index === 'BSX';
        const defaultLot = isBSE ? 20 : 65;
        const tradeQuantity = executionQty || defaultLot;

        const targetGateway = isBSE ? 'BSEFO' : 'NSEFO';
