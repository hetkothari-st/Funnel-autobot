...
const OriginalLayout = ({
    ...
    const handleQuickStrikes = () => {
        ...
        for (let i = 2; i >= 1; i--) ceStrikes.push(atm - i * step); // 2 ITM CE
        ceStrikes.push(atm);                                           // ATM
        for (let i = 1; i <= 5; i++) ceStrikes.push(atm + i * step);  // 5 OTM CE

        for (let i = 2; i >= 1; i--) peStrikes.push(atm + i * step); // 2 ITM PE
        peStrikes.push(atm);                                           // ATM
        for (let i = 1; i <= 5; i++) peStrikes.push(atm - i * step);  // 5 OTM PE
